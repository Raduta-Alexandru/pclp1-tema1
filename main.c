...
int** columnShifting(int** mat, int n, int m, 
	int columnNumber, int positionShift) {
...
int* calculatePanelScore(int** mat, int m) {
	...
	for(int i = 0; i < n; i++) {
		...
		for(int j = 0; j < m - 2; j++) {
			...
			//daca este mai mare decat maximul, inlocuiesc maxim cu noul maxim
...
int task1() {
	...
	mat = columnShifting(mat, n, m, columnNumber, positionShift);
	matrixWrite(mat, n, m);
	matrixFree(mat, n, m);
	return 0;
}
...
int task2() {
	...
	int* comboNrAndScore = malloc(2 * sizeof(int));
	if(comboNrAndScore == NULL) {
		exit(1);
	}
	comboNrAndScore = calculatePanelScore(mat, m);
...
int task5() {
	...
	for(int i = 0; i < m - 1; i++) {
		for(int j = i + 1; j < m; j++) {
			for(int k = 0; k < n; k++) {
				for(int l = 0; l < n; l++) {
					mat = columnShifting(mat, n, m, j ,1);
					comboNrAndScore = calculatePanelScore(mat, m);
					if(maxScore < comboNrAndScore[1]) {
						maxScore = comboNrAndScore[1];
					}
				}
				mat = columnShifting(mat, n, m, i, 1);
			}
		}
	}
	matrixFree(mat, n, m);
